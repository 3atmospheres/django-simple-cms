...
def digg_paginator(records, ADJACENT_PAGES=3, BORDER_PAGES=2, BASE_RANGE=10):
    ...
    if records.paginator.num_pages <= BASE_RANGE:
        ...
        if records.paginator.num_pages > 1:
            pages = range(1, records.paginator.num_pages+1)
    # in the first range, with trailing
    elif records.paginator.num_pages > BASE_RANGE and records.number < BASE_RANGE+1:
        pages = range(1, BASE_RANGE+1)
        trailing = range(records.paginator.num_pages-BORDER_PAGES+1, records.paginator.num_pages+1)
    # within the end range
    elif records.paginator.num_pages - records.number <= ADJACENT_PAGES*2 + 1:
        leading = range(1, BORDER_PAGES+1)
        pages = range(records.paginator.num_pages-(ADJACENT_PAGES*2+1), records.paginator.num_pages+1)
    else:
        leading = range(1, BORDER_PAGES+1)
        pages = range(records.number - ADJACENT_PAGES, records.number + ADJACENT_PAGES+1)
        trailing = range(records.paginator.num_pages-BORDER_PAGES+1, records.paginator.num_pages+1)
